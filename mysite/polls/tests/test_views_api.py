...
@pytest.fixture
def api_client():
    user = User.objects.create_user(username='testuser123', password='testpassword', is_staff=True, is_superuser=True)
...
class TestQuestionListAPI():
    ...
    def test_create_question(self, api_client):
        ...
        assert Question.objects.get().pub_date.strftime("%Y-%m-%dT%H:%M:%SZ") == '2023-08-28T12:00:00Z'
...
class TestQuestionDetailAPI():
    ...
    def test_get_question_detail(self, api_client):
        Question.objects.all().delete()
        question = Question.objects.create(question_text=faker.sentence(), pub_date='2023-08-28T12:00:00Z')
    ...
    def test_update_question(self, api_client):
        question = Question.objects.create(question_text='Question1', pub_date='2023-08-28T12:00:00Z')

        data = {'question_text': 'UpdatedText', 'pub_date': '2022-08-28T12:00:00Z'}
        response = api_client.put(reverse('api-question-detail', args=[question.pk]), data, format='json')
    ...
    def test_delete_question(self, api_client):
        question = Question.objects.create(question_text='Question1', pub_date='2023-08-28T12:00:00Z')
        response = api_client.delete(reverse('api-question-detail', args=[question.pk]), format='json')
